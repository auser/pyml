...
while ! nc -vz $($DM ip $CLUSTER_PREFIX) 22 -w 5; do
  sleep 2
done

R_DIR=/usr/local/src
$DM ssh $CLUSTER_PREFIX "sudo chown ubuntu $R_DIR"
$DM scp "$DIR/install-nvidia.sh" "$CLUSTER_PREFIX:$R_DIR/setup.sh"

if [[ "$DM ssh $CLUSTER_PREFIX 'stat $R_DIR/cudnn-7.0-linux-x64-v4.0-prod.tgz'" ]]; then
  echo "Not there... $R_DIR/cudnn-7.0-linux-x64-v4.0-prod.tgz"
  $DM scp "$DIR/../cudnn-7.0-linux-x64-v4.0-prod.tgz" "$CLUSTER_PREFIX:$R_DIR/"
else
  echo "File exists: $R_DIR/cudnn-7.0-linux-x64-v4.0-prod.tgz. Not uploading again..."
fi

$DM ssh $CLUSTER_PREFIX "chmod +x $R_DIR/setup.sh"
$DM ssh $CLUSTER_PREFIX "sudo $R_DIR/setup.sh"
