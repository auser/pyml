...
RUN BRANCH=$(v$(uname -r | sed -e "s/-.*//" | sed -e "s/\.[0]*$//"))
  git clone --depth 1 --branch stable $BRANCH git://git.kernel.org/pub/scm/linux/kernel/git/stable/linux-stable.git linux
WORKDIR linux
# RUN git checkout -b stable v$(uname -r | sed -e "s/-.*//" | sed -e "s/\.[0]*$//") &&
RUN zcat /proc/config.gz > .config && make modules_prepare
